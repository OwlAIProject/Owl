...
class ConversationEndpointDetector:
    ...
    def consume_samples(self, samples: torch.Tensor, end_stream: bool = False) -> List[Dict[str, int]]:
        if self._finished:
...
if __name__ == "__main__":
    ...
    filepath = "test.wav"
    format = "wav"
    audio = AudioSegment.from_file(filepath, format).set_channels(1).set_frame_rate(16000)
    #samples = torch.Tensor(audio.get_array_of_samples())
    samples, sr = torchaudio.load(filepath)
    samples = samples.squeeze()
    print(f"Num samples = {samples.numel()}")
    ...
    for i, chunk in enumerate(chunks):
        ts += svad.consume_samples(samples=chunk, return_milliseconds=False)
    ts += svad.consume_samples(samples=torch.Tensor([]), end_stream=True, return_milliseconds=False)
    print("Streaming:")
    print(ts)
    print("")
